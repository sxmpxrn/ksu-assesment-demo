...
import { ArrowLeft, User, GraduationCap, Loader2, Check } from "lucide-react";
...
export default function StudentListView({ room, onBack }: StudentListViewProps) {
    ...
    const viewRoomDetails = async (roomData: any) => {
        try {
            setSelectedRoomStudents(null); // clear first

            const supabase = createClient();
            const { data: studentsData, error } = await supabase
                .from("students")
                .select("id, student_id, first_name, last_name, gender, phone, confirm")
                .eq("room_id", roomData.id)
                .order("student_id");

            if (!error && studentsData) {
                setSelectedRoomStudents(studentsData);
            }
        } catch (e) {
            console.error(e);
        }
    ...
    const handleToggleConfirm = async (studentId: string, currentStatus: boolean) => {
        ...
        try {
            ...
            // Update state locally so UI reflects change immediately without re-fetching
            if (selectedRoomStudents) {
                setSelectedRoomStudents(
                    selectedRoomStudents.map((student) =>
                        student.id === studentId ? { ...student, confirm: newStatus } : student
                    )
                );
            }
        } catch (error) {
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-white border-b border-gray-100">
                                ...
                                <th className="px-8 py-4 text-xs font-black text-gray-400 uppercase tracking-widest whitespace-nowrap text-center">
                                    สถานะการยืนยัน
                                </th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50 bg-white">
                            {selectedRoomStudents === null ? (
                                ...
                                selectedRoomStudents.map((student: any, index: number) => {
                                    const isConfirmed = !!student.confirm;
                                    return (
                                        <tr
                                            key={student.id}
                                        ...
                                        >
                                            ...
                                            <td className="px-8 py-5">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="font-bold text-gray-800 text-sm">
                                                        <span
                                                            className={`mr-1 font-bold text-sm ${student.gender === "ชาย"
                                                                    ? "text-blue-600"
                                                                    : student.gender === "หญิง"
                                                                        ? "text-pink-500"
                                                                        : "text-gray-400"
                                            ...
                                            <td className="px-8 py-5 text-center">
                                                <button
                                                    onClick={() => handleToggleConfirm(student.id, isConfirmed)}
                                                    disabled={updatingStudentId === student.id}
                                                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#7ca3d5] focus:ring-offset-2 ${isConfirmed ? 'bg-emerald-500' : 'bg-gray-200 hover:bg-gray-300'
                                                        } ${updatingStudentId === student.id ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                                                >
                                                    <span className="sr-only">Toggle Confirm</span>
                                                    <span
                                                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform flex items-center justify-center ${isConfirmed ? 'translate-x-6' : 'translate-x-1'
                                                            }`}
                                                    >
                                                        {isConfirmed && <Check size={10} className="text-emerald-500" />}
                                                    </span>
                                                </button>
                                                <div className="mt-1">
                                                    {updatingStudentId === student.id ? (
                                                        <span className="text-[10px] font-bold text-gray-400">กำลังอัปเดต...</span>
                                                    ) : isConfirmed ? (
                                                        <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">ยืนยันแล้ว</span>
                                                    ) : (
                                                        <span className="text-[10px] font-bold text-gray-400">ยังไม่ยืนยัน</span>
                                                    )}
                                                </div>
                                            </td>
                                        </tr>
